...
activator_self = lambda x : natural_log(x) + keras.backend.relu(x)
...
model_3D = CNNs.threeDmodel(shadows[0].shape[0], shadows[0].shape[1], shadows[0].shape[2], activator_self)
...
model_KPCA = CNNs.twoDmodel(np.shape(X_train_K)[1], np.shape(X_train_K)[2], activator_self)
...
model_LDA = CNNs.twoDmodel(np.shape(X_train_diff)[1], np.shape(X_train_diff)[2], activator_self)
...
epoch_count = range(1, len(training_loss_KPCA) + 1)
...
plt.title("Logarithmic Loss vs Epochs; Split = %.2f; Activator = %s" %(split, activator_s))
plt.show()
